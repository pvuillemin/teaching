...
Qu'est ce que développer aujourd'hui?
Développer ("écrire du code"), c'est projeter en pensée puis dans un langage de programmation un schéma d'exécution, une suite d'instructions (un "algorithme") qui réponde à l'objectif fixé.\\
...
En C++, il vous faut à la fois déclarer et définir vos variables, vos fonctions (appelées méthodes dans la suite de ce cours) et vos classes. \\
...
Dans le cas des variables, on peut effectuer ces deux opérations en une seule ligne \footnote{en une seule ligne, mais pas de manière atomique. Ceci dépasse cependant le cadre de ce chapitre} de la manière suivante :\\
...
Dans le cas des méthodes, la déclaration consiste en la donnée des informations suivantes : \textit{nom de la méthode}, \textit{scope de la méthode} (nous y reviendrons), \textit{type et nombre des arguments}, \textit{type de retour}, et éventuellement d'autres informations pour caractériser l'usage de la méthode (mots-clef static/extern etc...). Voici un exemple de déclaration de méthodes :\\
...
Nous déclarons ainsi une méthode Pow, qui prend deux arguments, l'un réel (type: double) et l'autre entier (type: int), et qui retourne un double.
Nous pouvons maintenant définir cette méthode, c'est à dire instruire le compilateur de ce qu'elle fait exactement:\\
...
Lorsque vous faites référence dans un fichier B à une méthode \underline{déclarée} dans un fichier A.h, il est nécessaire que le compilateur sache où trouver cette méthode. Le compilateur n'a pas besoin dans un premier temps d'en connaître la définition, mais il doit pouvoir accéder à sa déclaration : vous vous devez donc d'informer le compilateur qu'il pourra aller chercher des déclarations à l'intérieur du fichier A.h. Ceci se réalise grâce à la commande \#include qui sera insérée en en-tête du fichier B:\\
...
Au niveau de la précompilation, une opération est réalisée de telle sorte que le compilateur considère que le code du fichier A.h se trouve au début du fichier B, à l'endroit où se trouvait l'instruction \#include, ce qui va permettre au compilateur de pouvoir accéder dans B à la déclaration de la méthode déclarée dans A.h. Tout se passe donc "comme si" le contenu du fichier A.h était copié dans le fichier B. Cette étape est représentée dans la figure \ref{fig:preprocessing1}.\\
...
La commande \#define est initialement utilisée pour faire des substitutions de chaînes de caractères à l'intérieur du fichier dans lequel la commande \#define est écrite. Le pré-processeur va donc parcourir le fichier et remplacer toutes les occurences de la variable ainsi définie. Ainsi, la commande :\\
