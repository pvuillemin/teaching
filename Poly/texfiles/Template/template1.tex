...
\qauthor{propos prêté faussement à Edsger Dijkstra, réel auteur inconnu}
\end{savequote}
...
Nous le concevons donc, \textbf{la redondance du code est \`a proscrire}\footnote{Une règle importante du d\'eveloppement pourrait \^etre : "ne faites pas de copier/coller de code au sein d'un projet, sauf éventuellement si celà vous évite de devoir inclure des dépendances supplémentaires."}. Comment dans ces conditions cr\'eer une seule fonction Max qui permette de d\'efinir cette fonction
...
remplac\'e par \textit{class}\footnote{En réalité, les deux mots clefs ne sont parfaitement substituables l'un à l'autre que dans les cas simples qui nous intéressent. Pour en savoir plus, nous renvoyons le lecteur par exemple à \cite{Lippman}.}.\\
...
Nous renvoyons le lecteur désireux de s'initier à l'inlining par exemple à \cite{Thinking}.\\
...
Cette fonction fonctionnerait également si elle était spécifiée en deux entiers. Cependant, dans le cas où \varname{y} est entier, il n'est pas nécessaire de passer par cette formule, il suffit alors de multiplier 2 par lui même \varname{y} fois. Bien que la première formule soit exacte même dans le cas où \varname{y} est entier, elle entrainerait donc des calculs indûment longs. Pour améliorer cette situation, nous voudrions dire au compilateur : compile la fonction précédente pour tous les types nécessaires, SAUF dans le cas où \varname{y} est entier, auquel cas contente toi de calculer directement la valeur de l'exponentiation par la deuxième méthode. En C++, il est possible de préciser/redéfinir une spécialisation spécifique.\\
...
Il est également possible de paramétrer une fonction par autre chose qu'un type. Notamment, il est possible de paramétrer une fonction par un entier. Ces mécanismes ne seront pas détaillés dans ce cours, mais ils sont massivement utilisés dans de nombreuses librairies professionnelles, car ils permettent des optimisations très fines, notamment via le Template Meta-Programming. Les bonnes librairies de calcul scientifique en C++ par exemple reposent toutes sur ce genre d'optimisation. Nous renvoyons le lecteur intéressé par exemple à \cite{Alexandrescu}. 