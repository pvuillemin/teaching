...
Dans ce chapitre, nous raffinons le modèle mémoire déjà ébauché en section \ref{sec:stack} en introduisant une nouvelle forme de mémoire, la \textit{Heap}, à la fois plus souple et plus complexe.
...
Nous donnons tout d'abord deux cas dans lesquels nous ne pouvons/souhaitons pas recourir à cette forme de mémoire.\\
...
r\'eserver n cases m\'emoires, \emph{pendant l'ex\'ecution et non pas à la compilation} du programme " ? Il
existe une fonctionalité\footnote{Pour \^etre plus pr\'ecis, il s'agit en fait d'un op\'erateur d'un genre un peu particulier.} en C++ appel\'ee \texttt{new}, qui effectue pr\'ecis\'ement
...
\footnote{appel\'e ramasse-miette, ou \textit{garbage collection}} de la mémoire qui n'est plus utilisée,
mais ce n'est pas le cas du C++ \footnote{Au moins à notre niveau}.\\

Afin d'\'eviter ce genre de situation, la mémoire allouée dans un programme doit \emph{toujours} être lib\'erée dès qu'elle n'est plus utilisée. Cela se fait
au moyen de l'instruction \texttt{delete}. Cette instruction prend deux formes:

\begin{itemize}
	\item Si la mémoire allouée était destinée à un tableau, elle doit être désallouée avec l'opérateur delete[] :
\begin{lstlisting}
delete[] someArray;
\end{lstlisting}
	\item Si la mémoire allouée était destinée à une variable, on utilisera une forme sans crochet:
\begin{lstlisting}
delete pSomeElement;
\end{lstlisting}
\end{itemize}
...
\`A chaque fois que l'on \'ecrit un \functionname{new}, il faut \'ecrire immédiatement le
